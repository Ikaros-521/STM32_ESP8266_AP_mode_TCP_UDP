...
PRESTA:
    ...
    printf("连接状态:"); 		//连接状态
    printf("STA模式:"); 		//STA连接状态
    printf("AP模式:"); 		//AP连接状态
    printf("发送数据:"); 	//发送数据
    printf("接收数据:");	//接收数据
    atk_8266_wificonf_show(15,195,"请设置路由器无线参数为:",(u8*)wifista_ssid,(u8*)wifista_encryption,(u8*)wifista_password);
    POINT_COLOR=BLUE;
    printf("%s",(u8*)ATK_ESP8266_WORKMODE_TBL[netpro&0X03]); 		//STA连接状态
    printf("%s",(u8*)ATK_ESP8266_WORKMODE_TBL[netpro>>4]); 		//AP连接状态
    USART3_RX_STA=0;
    while(1)
    {
        key=KEY_Scan(0);
